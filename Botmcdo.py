...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for texte, resultat, date in all_pronos:
        if resultat == "attente":
            en_cours += 1
            status = "⏳ En cours"
        elif resultat and any(word in resultat.lower() for word in ["gagn", "gagné", "win", "victoire", "1", "2", "3", "oui", "marquent"]):
            gagnés += 1
            status = "✅ Gagné"
        else:
            perdus += 1
...
async def set_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if rows_updated > 0:
            await update.message.reply_text(f"✅ Résultat mis à jour avec succès :\n\n**{resultat}**", parse_mode='Markdown')
        else:
            await update.message.reply_text("⚠️ Aucun prono trouvé à mettre à jour.")
