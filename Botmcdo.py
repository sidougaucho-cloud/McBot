...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute("SELECT COUNT(CASE WHEN vote='oui' THEN 1 END) as oui, COUNT(CASE WHEN vote='non' THEN 1 END) as non FROM prono_votes")
    votes = c.fetchone()
    oui = votes[0] or 0
    non = votes[1] or 0
    total = oui + non
    ...
    await update.message.reply_text(
        ...
        f"❌ Pas d’accord : {non}\n"
        f"📊 Total votes : {total}\n\n"
        f"👥 Participants pronos : {participants}",
        parse_mode='Markdown'
    )
    conn.close()
...
if __name__ == "__main__":
    ...
    print("🚀 Bot lancé avec succès - Système de vote ajouté !")
    app.run_polling()
