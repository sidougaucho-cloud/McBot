...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ticket_jour":
        ...
        row = c.fetchone()
        conn.close()
        texte = row[0] if row else "Aucun ticket du jour."
...
# ================= SET RESULT (CORRIGÉ ET RENFORCÉ) =================
async def set_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Erreur lors de la mise à jour : {e}")
...
async def set_prono(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id == ADMIN_ID:
        ...
        await update.message.reply_text(f"✅ Prono publié et envoyé à {sent} abonnés !")
    else:
