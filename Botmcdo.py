...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username = query.from_user.username or "Aucun"
    ...
    if data == "menu_prono_prono":
        if not is_vip(user_id):
            await query.edit_message_text("❌ **Accès VIP requis**\n\nLe Prono du jour est réservé aux abonnés VIP.", 
                                          reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⭐ S'abonner au VIP", callback_data="menu_vip")], [InlineKeyboardButton("⬅️ Retour", callback_data="back")]]))
            return
        # ... reste du code prono ...
    ...
    if data.startswith("buy_"):
        ...
        await query.edit_message_text(
            f"🛒 **{produit}**\n\n💰 Prix : **{prix}€**\n\n🔗 Clique sur le lien pour payer :\n{paypal_link}\n\n📸 Une fois payé, clique sur le bouton ci-dessous.",
            parse_mode="Markdown",
...
if __name__ == "__main__":
    ...
    print("🚀 Bot démarré - Notification admin activée")
    app.run_polling()
