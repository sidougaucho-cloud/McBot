...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # SHOP (tout le reste du button_handler est inchangé)
...
# ================= STATS AMÉLIORÉE : Historique Gagné / Perdu en % =================
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    conn = sqlite3.connect("bot.db")
    c = conn.cursor()

    # Récupération de tous les pronos du jour terminés (sauf "attente")
    ...
    for texte, resultat, date in all_pronos:
        if resultat == "attente":
            en_cours += 1
            status = "⏳ En cours"
        elif resultat and any(word in resultat.lower() for word in ["gagn", "gagné", "win", "1", "2", "oui", "marquent", "victoire"]):
            gagnés += 1
            status = "✅ Gagné"
        else:
            perdus += 1
...
if __name__ == "__main__":
    ...
    print("🚀 Bot lancé avec succès - Stats Gagné/Perdu avec pourcentages !")
    app.run_polling()
