...
🔔 /notif on  → Recevoir une notification à chaque nouveau prono du jour
🔔 /notif off → Désactiver les notifications"""
...
async def handle_proof(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ADMIN_ID,
        f"🛒 Preuve reçue pour commande #{cid}",
...
async def notif(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🔔 Notifications du prono du jour **{status}**.")


# ================= SET PRONO (avec envoi des notifications) =================
    ...
    if update.effective_user.id == ADMIN_ID:
        ...
        for (user_id,) in subscribers:
            try:
                await context.bot.send_message(
                    user_id,
                    f"🔔 **Nouveau Prono du Jour !**\n\n{texte}\n\nVa dans le menu Pronostics → Prono du jour pour voter."
                )
                success += 1
            except:
                pass  # utilisateur bloqué ou bot bloqué

        await update.message.reply_text(f"✅ Prono du jour mis à jour et envoyé à {success} abonnés !")
    else:
        # Prono personnel utilisateur
        ...
        c.execute("DELETE FROM user_pronos WHERE user_id = ? AND match = ?", 
                  (update.effective_user.id, match))
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    gagnés = sum(1 for _, r, _ in all_pronos if r and any(w in (r or "").lower() for w in ["gagn", "win", "victoire", "1", "2", "3", "oui", "marquent"]))
    perdus = sum(1 for _, r, _ in all_pronos if r and not any(w in (r or "").lower() for w in ["gagn", "win", "victoire", "1", "2", "3", "oui", "marquent"]) and r != "attente")
    en_cours = total - gagnés - perdus
    ...
    await update.message.reply_text(
        f"📊 **Statistiques Pronos**\n\n"
        f"Total : {total} | En cours : {en_cours}\n"
        f"✅ Gagnés : {gagnés} ({p_g}%)\n"
        f"❌ Perdus : {perdus} ({p_p}%)",
        parse_mode='Markdown'
    )
...
if __name__ == "__main__":
    ...
    app.add_handler(CommandHandler("notif", notif))   # ← Nouvelle commande

    print("🚀 Bot lancé avec succès - Notifications prono du jour activées !")
    app.run_polling()
