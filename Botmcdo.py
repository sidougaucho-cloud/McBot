...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_prono_prono":
        ...
        texte = row[0] if row else "Aucun prono"
        resultat = f"\n\n**Résultat :** {row[1]}" if row and row[1] else ""
...
async def set_prono(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    match = parts[0]
    prono = parts[1] if len(parts) > 1 else ""
    cote = float(parts[2]) if len(parts) > 2 else None

    conn = sqlite3.connect("bot.db")
    c = conn.cursor()
    c.execute("DELETE FROM user_pronos WHERE user_id = ? AND match LIKE ?", (user.id, f"%{match}%"))
    c.execute("INSERT INTO user_pronos (user_id, username, match, prono, cote) VALUES (?,?,?,?,?)",
              (user.id, user.username or "user", match, prono, cote))
    conn.commit()
    conn.close()

    await update.message.reply_text(f"✅ Prono enregistré !\n\n{match}\n{prono} @ {cote if cote else '?'}")
...
async def set_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        ...
        return
    if not context.args:
        await update.message.reply_text("`/setresult gagné` ou `perdu` ou `2-1`")
        return
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📊 **Stats**\n\nProno : {prono}\nRésultat : {res}\nParticipants : {participants}", parse_mode='Markdown')
    conn.close()
...
async def mypronos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect("bot.db")
    c = conn.cursor()
    c.execute("SELECT match, prono, cote FROM user_pronos WHERE user_id = ? ORDER BY date DESC", (user.id,))
    rows = c.fetchall()
    conn.close()
    if not rows:
        await update.message.reply_text("Tu n'as aucun prono.")
        return
    text = "📋 **Tes pronos :**\n\n"
    for m, p, c in rows:
        text += f"• {m}\n  → {p} @ {c if c else '?'}\n\n"
...
if __name__ == "__main__":
    ...
    print("🚀 Bot démarré avec succès - /setprono, /setresult, /stats et /mypronos fonctionnels")
    app.run_polling()
